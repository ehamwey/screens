...
You will also need to install `direnv` from `brew` and load the api key from `.envrc` using `direnv allow`. If this is your first time running `screens` copy the `.envrc-template` to `.envrc` and replace it with your api key. 
